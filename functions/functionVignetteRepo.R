functionVignetteRepo <- function(importFuns, 
                             funSub, 
                             plotName, 
                             ylims, mainYlims){
...
tempGam <- gam(cbind(count, fail) ~ s(monthNum), data=importDf, family=binomial, method="REML")

pred.df <- data.frame(monthNum=seq(min(monthNum), max(monthNum), 1))
pred.df <- cbind(pred.df,
             as.data.frame(predict(tempGam, newdata=pred.df, se.fit=TRUE)))

changeRatio <- mean(plogis(pred.df$fit)[nrow(pred.df) - (0:11)]) /mean(plogis(pred.df$fit)[1:12])
...
axis(side=2, at=c(0,0.5,1), line=-4)
